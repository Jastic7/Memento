...
static RemoteTransportLayer *sharedInstance = nil;

@interface RemoteTransportLayer ()
...
@implementation RemoteTransportLayer
...
- (void)obtainDataWithParameters:(NSDictionary<NSString *,NSString *> *)parameters
    ...
    FIRDatabaseReference *requestRef = [self databasePathWithParameters:parameters];
...
- (FIRDatabaseReference *)databasePathWithParameters:(NSDictionary *)parameters {
    NSString *dataType = parameters[@"dataType"];
    NSString *ownerId  = parameters[@"ownerId"];
    NSString *dataId   = parameters[@"dataId"];
    
    FIRDatabaseReference *ref =  [self.rootRefDB child:dataType];
    if (ownerId) {
        ref = [ref child:ownerId];
    }
    
    if (dataId) {
        ref = [ref child:dataId];
    }
